...
#define dynarr_resize_nf(da, n) \
	do { \
		if(!((da) = dynarr_resize((da), (n)))) { \
			cgi_panic("failed to resize dynamic array\n"); \
		} \
	} while(0)
#define dynarr_clear_nf(da) \
	do { \
		if(!((da) = dynarr_clear(da))) { \
			cgi_panic("failed to clear dynamic array\n"); \
		} \
	} while(0)
#define dynarr_push_nf(da, item) \
	do { \
		if(!((da) = dynarr_push((da), (item)))) { \
			cgi_panic("failed to append to dynamic array\n"); \
		} \
	} while(0)
#define dynarr_pop_nf(da) \
	do { \
		if(!((da) = dynarr_pop(da))) { \
			cgi_panic("failed to remove from dynamic array\n"); \
		} \
	} while(0)
